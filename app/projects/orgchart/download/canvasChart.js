...
export default class CanvasChart {
  ...
  computedCanvasSize() {
    ...
    if (this.canvasWidth * this.ratio >= this.MAX_SIZE) {
      this.scale =
        Math.ceil(((this.canvasWidth * this.ratio) / this.MAX_SIZE) * 10000) /
        10000;
    }

    if (this.canvasHeight * this.ratio >= this.MAX_SIZE) {
      this.scale =
        Math.ceil(((this.canvasHeight * this.ratio) / this.MAX_SIZE) * 10000) /
        10000;
    }
  }
  ...
  async initPaint() {
    ...
    await Promise.all(
      this.config.nodeList.map(async (node) => await this.paintNode(node))
    );
  }
  ...
  getNodeRealLoc(node) {
    ...
    return [
      node.x + currOffsetX + this.offsetX,
      node.y + currOffsetY + this.offsetY,
    ];
  }
  ...
  paintText(card, node, x, y) {
    const textInfo = resolveParams(card.text, "", { node }).reduce(
      (t1, t2) => t1 + t2,
      ""
    );
      ...
      case "tb":
        textList.map((textObj) => {
          ...
          for (let iy = 0; iy < textstr.length; iy++) {
            const currWidth = this.getChartWidth(
              textstr[iy],
              fontSize,
              fontWeight
            );
  ...
  paintText_fullText(card, node, x, y) {
    const textInfo = resolveParams(card.text, "", node.data).reduce(
      (t1, t2) => t1 + t2,
      ""
    );
    const fontSize = card.fontSize / this.scale;
  ...
  computedIconFollow(cardModel, node) {
    ...
    switch (cardModel.tag) {
      case "text":
        ...
        textLen = getStrWidth(
          textInfo,
          cardModel.fontSize / this.scale,
          cardModel.fontWeight
        );
  ...
  getTextList(textInfo, fontSize, fontWeight, lineWidth) {
    ...
    for (let i = 0; i < textInfo.length; i++) {
      ...
      if (
        currLineWidth +
          this.getChartWidth(textInfo[i + 1] || "", fontSize, fontWeight) >
        lineWidth
      ) {
        textList.push({
          text: currText,
...
async function loadimg(src) {
  return new Promise((resolve) => {
    ...
    img.src =
      src + `?timestamp=${Date.now() + parseInt(Math.random() * 99999999)}`;
