...
function getTextXml(textObj) {
  if (textObj.text) {
    ...
    textXml += textObj.options.fontSize
      ...
      : "";
    textXml += textObj.options.hasOwnProperty("bold")
      ? ` b="${textObj.options.bold ? 1 : 0}"`
      : "";
...
function getTextBody(slideObj) {
  ...
  if (opts && (typeof slideObj.text === "undefined" || slideObj.text === null))
    return "";
  let tmpTextObjects = [];
  let arrTextObjects = [];
  ...
  slide._slideObjects.forEach((slideItemObj, idx) => {
    ...
    switch (slideItemObj._type) {
      ...
      case "group":
        strSlideXml += `<p:grpSp>
                <p:nvGrpSpPr>
                    <p:cNvPr id="${9999999}" name="${
          slideItemObj.options.shapeName
        }">
                        <a:extLst>
...
function getPathStr(slideItemObj, x, y, cx, cy, elbowHeight) {
  ...
  // flipH 水平翻转
  let strSlideXml = "";
  let { tx, ty, h, w, sidx, eidx } = rotateLoc(
    x,
    y,
    cx,
    cy,
    slideItemObj.options.line.linkType
  );
...
function getTextStr(slideItemObj, x, y, cx, cy, idx) {
  ...
  strSlideXml +=
    "</a:avLst></a:prstGeom>" +
    (slideItemObj.options.fill
      ? getColoXml(slideItemObj.options.fill)
      : "<a:noFill/>");
  strSlideXml += `${getLineXml(slideItemObj)}</p:spPr>${getTextBody(
    slideItemObj
  )}</p:sp>`;
  return strSlideXml;
}
...
function getImgStr(slideItemObj, x, y, cx, cy, idx) {
  ...
  strSlideXml += `<p:cNvPr id="${idx + 2}" name="Object ${
    idx + 2
  }" descr="${encodeXmlEntities(imageOpts.altText || slideItemObj.image)}">`;
  strSlideXml += "    </p:cNvPr>";
  strSlideXml +=
...
function getLineXml(slideItemObj) {
  ...
  if (slideItemObj.options.line) {
    ...
    if (slideItemObj.options.line.color)
      xml += getColoXml(slideItemObj.options.line);
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes">
	...
	<TitlesOfParts>
		<vt:vector size="${slides.length + 3}" baseType="lpstr">
			...
			${slides
        .map(
          (_slideObj, idx) => "<vt:lpstr>Slide " + (idx + 1) + "</vt:lpstr>\n"
        )
        .join("")}
		</vt:vector>
	</TitlesOfParts>
...
function getPresentationXml(pres) {
  ...
  }"/><p:notesSz cx="${pres.presLayout.height}" cy="${
    pres.presLayout.width
  }"/>`;
...
function getMasterXml(slide, layouts) {
  let layoutDefs = layouts.map(
    (_layoutDef, idx) =>
      `<p:sldLayoutId id="${2147483649 + idx}" r:id="rId${idx + 1}"/>`
  );
...
function setShapeObj(shapeName, options) {
  ...
  options.elbowRatio =
    options.elbowRatio || (options.elbowRatio === 0 ? 0 : 0.5);
  options.elbowHeight =
    options.elbowHeight || (options.elbowHeight === 0 ? 0 : 0.3);
...
function setTextObj(text, options) {
  ...
  if ((options.valign || "").toLowerCase().indexOf("b") === 0)
    options._bodyProp.anchor = "b";
  else if ((options.valign || "").toLowerCase().indexOf("m") === 0)
    options._bodyProp.anchor = "ctr";
  else if ((options.valign || "").toLowerCase().indexOf("t") === 0)
    options._bodyProp.anchor = "t";
  let textObj = {
    _type: "text",
...
function getNewRelId(target) {
  return (
    target._rels.length +
    target._relsChart.length +
    target._relsMedia.length +
    1
  );
}
  ...
  downloadPpt(props) {
    ...
    return Promise.all(arrMediaPromises).then(() => {
      ...
      zip.file(
        ...
        getPresentationRels(this.slides)
      );
