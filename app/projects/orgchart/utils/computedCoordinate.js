...
function conputedChildLeafLoc(root, baseCardSize, baseLocMap, heightMap) {
  ...
  return (
    Math.max(rightLen, leftLen) * (baseCardSize.h + baseCardSize.ySpace) -
    baseCardSize.ySpace2v / 2
  );
}
...
function getBaseLoc(treeData, baseCardSize, heightMap) {
  ...
  const tree = d3
    .tree()
    .nodeSize([
      baseCardSize.w + baseCardSize.xSpace,
      baseCardSize.h + baseCardSize.ySpace,
    ]);
...
function getAllVLocMap(treeData, baseCardSize, changeLevel) {
  ...
  const tree = d3
    .tree()
    .nodeSize([
      baseCardSize.h + baseCardSize.xSpace2r,
      baseCardSize.w + baseCardSize.ySpace,
    ]);
...
function seatTree2v(tree, baseCardSize, changeLevel) {
  ...
  function each(list, level) {
    ...
    let addCurrNum = Math.ceil(
      baseCardSize.w / (baseCardSize.h + baseCardSize.xSpace2r)
    );
